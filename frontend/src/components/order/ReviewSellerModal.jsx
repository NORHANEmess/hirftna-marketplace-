...
export default function ReviewSellerModal({ order, onClose, onSuccess }) {
  ...
  const sellerName = order.seller?.shop_name ?? t('orders.card.artisanFallback');

  async function handleSubmit() {
    if (!rating) {
      setError(t('reviewSeller.ratingRequired'));
      return;
    }
    ...
    >
      <div className="w-full max-w-sm bg-white rounded-3xl shadow-xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-3">
          <div>
            <h2 className="text-base font-bold text-warm-900">{t('reviewSeller.title')}</h2>
            <p className="text-xs text-warm-400 mt-0.5">{sellerName}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-5 pb-5 space-y-4">
          {done ? (
            <div className="text-center py-6">
              ...
              <p className="font-bold text-warm-900">{t('reviewSeller.success')}</p>
              <p className="text-xs text-warm-400 mt-1">{t('reviewSeller.successSub')}</p>
            </div>
          ) : (
            <>
              {/* Star picker */}
