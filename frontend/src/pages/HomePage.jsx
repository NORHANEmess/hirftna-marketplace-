...
import { ArrowRight, CheckCircle2, ChevronRight, MapPin, ShoppingBag, Star } from 'lucide-react';
import { extractApiItems, productsAPI, sellersAPI } from '../services/api';
...
function SellerCard({ seller }) {
  ...
  return (
    <div
      ...
      className="bg-white rounded-2xl border border-beige-200 overflow-hidden cursor-pointer hover:shadow-md hover:border-sage-200 hover:-translate-y-0.5 transition-all duration-200 flex flex-col"
    >
      <div className={`h-20 bg-gradient-to-br ${gradients[gradientIndex]} relative flex-shrink-0`}>
        {seller.cover_url && <img src={seller.cover_url} alt="" className="w-full h-full object-cover" />}
      </div>
...
function FeaturedProductCard({ product }) {
  ...
  if (product.is_new) {
    badges.push({ label: 'New', color: 'bg-sage-600 text-white' });
  }
  if (product.is_featured) {
    badges.push({ label: 'Featured', color: 'bg-warning text-white' });
  }

  const price = product.price_min
    ? `DZD ${Number(product.price_min).toLocaleString()}`
    : product.price
      ? `DZD ${Number(product.price).toLocaleString()}`
      : null;
...
function ProductFilterTabs({ active, onChange }) {
  const tabs = [
    { value: 'all', label: 'All', sort: 'newest' },
  ...
  const pillars = [
    {
      ...
      body: 'Hirftna is an Algerian marketplace where people come together to make, sell, buy and discover unique handmade items.',
      link: 'See the artisans behind the work.',
      href: '/browse',
    },
    {
      title: 'Support for independent creators',
      body: 'There is no warehouse here. Buyers connect directly with artisans and commission pieces built with intention and skill.',
    },
    {
      title: 'Peace of mind, guaranteed',
      body: 'We focus on authenticity, verified sellers, and custom requests that reflect what buyers actually want.',
    },
  ];
  ...
  return (
    <section className="mt-14 border-t border-beige-200 pt-12 px-4 md:px-0">
      ...
      <div className="mt-10 text-center">
        <p className="text-sm font-bold text-warm-900 mb-4">You have a question? Great, we have answers.</p>
        <button
          type="button"
          onClick={() => navigate('/browse')}
          className="inline-flex items-center gap-2 px-7 py-2.5 border-2 border-warm-900 rounded-full text-sm font-semibold text-warm-900 hover:bg-warm-900 hover:text-white transition-all duration-200"
        >
...
export default function HomePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([productsResponse, sellersResponse]) => {
        setProducts(extractApiItems(productsResponse, { itemKeys: ['products'] }));
        setSellers(extractApiItems(sellersResponse, { itemKeys: ['sellers'] }));
      })
      .catch(() => {
        setProducts([]);
        setSellers([]);
          ...
          <>
            ...
            <div className="flex justify-center mt-6">
              <button
                type="button"
                onClick={() => navigate('/browse')}
                className="inline-flex items-center gap-2 px-6 py-2.5 border border-beige-200 bg-white text-sm font-semibold text-warm-800 rounded-full hover:border-sage-400 hover:text-sage-700 transition-all duration-200 shadow-sm"
              >
