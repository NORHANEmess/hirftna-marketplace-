...
const ensureArray = (items) => {
  ...
  if (!Array.isArray(items)) {
    throw new Error(
      `Expected an array response payload, received: ${typeof items}`
    );
  }
...
const buildPaginationMeta = (pagination) => {
  if (!pagination || typeof pagination !== 'object') {
    throw new Error(
      'A pagination object { page, limit, total } is required for list responses'
    );
  }
...
const sendCollection = (
  ...
  if (pagination) {
    data.pagination = buildPaginationMeta(pagination);
  }

  aliases.forEach((alias) => {
    if (alias && alias !== 'items') {
      data[alias] = safeItems;
    }
  });
